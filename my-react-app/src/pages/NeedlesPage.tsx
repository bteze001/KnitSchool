...
// ── Image imports — update these paths to match your actual file locations ──
import imgStraight        from "@/assets/straight.png";
import imgCircular        from "@/assets/circular.png";
import imgDpn             from "@/assets/double.png";
import imgInterchangeable from "@/assets/interchangable.png";
import imgAluminum        from "@/assets/aluminum.png";
import imgErgonomic       from "@/assets/ergonomic.webp";
import imgSteel           from "@/assets/steel_hooks.png";
import imgTunisian        from "@/assets/tunisian.png";

const TOOL_IMAGES: Record<string, string> = {
  straight:          imgStraight,
  circular:          imgCircular,
  dpn:               imgDpn,
  interchangeable:   imgInterchangeable,
  "aluminum-hook":   imgAluminum,
  "ergonomic-hook":  imgErgonomic,
  "steel-hook":      imgSteel,
  "tunisian-hook":   imgTunisian,
};

// ── Shared image component ───────────────────────────────────────────────────
function NeedleImage({ id, name, size = "card" }: { id: string; name: string; size?: "card" | "hero" }) {
  const src = TOOL_IMAGES[id];
  const height = size === "hero" ? 240 : 160;
  if (!src) return null;
  return (
    <div
      className="w-full bg-transparent border-b border-border/40 overflow-hidden flex items-center justify-center"
      style={{ height }}
    >
      <img
        src={src}
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-12">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        <Link to="/needles" className="text-sm text-muted-foreground hover:text-foreground mb-6 block">
          ← All Needles & Hooks
        </Link>
        ...
        {/* Hero image */}
        <div className="bg-card rounded-card border border-border/50 shadow-soft overflow-hidden mb-8">
          <NeedleImage id={needle.id} name={needle.name} size="hero" />
          <p className="px-5 py-3 text-xs text-muted-foreground italic">{needle.imageDescription}</p>
        </div>
    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-muted-foreground mb-8">
          Everything you need to know about knitting needles and crochet hooks.
        </p>
      </motion.div>
        ...
        {filtered.map((n, i) => (
          ...
          >
            <Link to={`/needles/${n.id}`}>
              <div className="bg-card rounded-card shadow-soft hover:shadow-hover transition-all border border-border/50 hover:-translate-y-1 overflow-hidden">
                <NeedleImage id={n.id} name={n.name} size="card" />
                <div className="p-5">
                  <span className="text-xs font-semibold uppercase tracking-wider text-secondary">
                    {n.category}
                  </span>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {n.bestFor.slice(0, 3).map((b) => (
                      <span key={b} className="px-2 py-0.5 bg-accent text-accent-foreground rounded-full text-xs">
                        {b}
                      </span>
                    ))}
                  </div>
